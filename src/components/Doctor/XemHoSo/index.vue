...
export default {
  ...
  methods: {
    locKhachHang() {
      const keyword = this.searchKhachHang.trim().toLowerCase();
      if (!keyword) {
        this.khachHangGoiY = [];
        return;
      }
      this.khachHangGoiY = this.customers.filter(kh => {
        return (
          kh.ho_va_ten.toLowerCase().includes(keyword) ||
          kh.so_dien_thoai.includes(keyword)
        );
      });
    },
    anDropdown() {
    ...
    chonKhachHang(kh) {
      ...
      this.searchKhachHang = `${kh.ho_va_ten} (${kh.so_dien_thoai})`;
      this.khachHangGoiY = [];
      this.hienDropdown = false;
