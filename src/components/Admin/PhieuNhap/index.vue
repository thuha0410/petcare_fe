                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr v-if="listPhieu.length === 0">
                            <td colspan="6" class="text-center fst-italic text-muted">Không có dữ liệu</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select v-model="phieuNhap.id_kho" class="form-select"
                                    ...
                                    <option v-for="k in listKho" :key="k.id" :value="k.id">{{ k.ten_kho }}</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                ...
                                <select v-model="phieuNhap.id_ncc" class="form-select"
                                    ...
                                    <option v-for="n in listNCC" :key="n.id" :value="n.id">{{ n.ten_ncc }}</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                    ...
                                    <td>
                                        <select v-model="item.id_thuoc" class="form-select">
                                            ...
                                            <option v-for="thuoc in listThuoc" :key="thuoc.id" :value="thuoc.id"
                                                :disabled="isThuocDaChon(thuoc.id, index)">{{ thuoc.ten_thuoc }}
                                            </option>
                                        </select>
                                    </td>
                                    <td><input type="number" v-model.number="item.so_luong" min="1" class="form-control" @input="triggerUpdate" /></td>
                                    <td><input type="number" v-model.number="item.gia_nhap" min="1" class="form-control" @input="triggerUpdate" /></td>
                                    <td><input type="date" v-model="item.han_su_dung" class="form-control" /></td>
                                    <td>{{ (Number(item.so_luong) * Number(item.gia_nhap)).toLocaleString() }}</td>
                                    <td><button class="btn btn-danger btn-sm" @click="xoaDong(index)">Xoá</button></td>
                                </tr>
                                <tr>
                                    <td colspan="4" class="text-end fw-bold">Tổng tiền:</td>
                                    <td colspan="2" class="fw-bold text-danger">{{ tinhTongTien().toLocaleString() }} VND</td>
                                </tr>
                            </tbody>
                        </table>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button class="btn btn-success" @click="luuPhieuNhap">Lưu phiếu nhập</button>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
  data() {
    return {
      ...
      chiTiet: [
        { id_thuoc: '', so_luong: 1, gia_nhap: 0, han_su_dung: '' }
      ],
  ...
  methods: {
    ...
    themDong() {
  const newItem = { id_thuoc: '', so_luong: 1, gia_nhap: 0, han_su_dung: '' };
  ...
  console.log(this.chiTiet);
    ...
    isThuocDaChon(id, currentIndex) {
      return this.chiTiet.some((item, idx) => idx !== currentIndex && item.id_thuoc === id);
    },
    luuPhieuNhap() {
      ...
      for (let i = 0; i < this.chiTiet.length; i++) {
        ...
        if (!item.id_thuoc || !item.so_luong || !item.gia_nhap || !item.han_su_dung) {
      ...
      axios.post('http://127.0.0.1:8000/api/phieu-nhap/tao', data)
        .then((res) => {
          if (res.data.status) {
