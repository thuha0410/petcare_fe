<template>
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between">
          <button class="btn btn-secondary" @click="moModalThuoc">
            + Thêm thuốc
          </button>
          <button class="btn btn-primary" @click="luuPhieu">
            Lưu phiếu nhập
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    luuPhieu() {
      if (
        !this.phieuNhap.id_kho ||
        !this.phieuNhap.id_ncc ||
        !this.phieuNhap.ngay_nhap ||
        this.chiTiet.length === 0
      ) {
      ...
      axios
        .post("http://127.0.0.1:8000/api/phieu-nhap/tao", dataGui)
        .then((res) => {
          if (res.data.status) {
            toaster.success(res.data.message);
            this.resetPhieu();
